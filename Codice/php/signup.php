<?php
    ...
    if(!isset($_POST['new_user_submit'])) {
        ...
        $page = str_replace("<error/>", "<div class=\"errors-forms\"><p> Si è verificato un errore clicca <a href=\"../index.php\">qui</a>  per tornare allla home</p></div>", $page); 
    ...
    if (strlen($email) == 0){
        $error_messages .= '<li>Email non inserita</li>';
    }

    if (strlen($password) == 0){
        $error_messages .= '<li>Password non inserita</li>';
    }

    if (strlen($password_confirm) == 0){
        $error_messages .= '<li>Conferma password non inserita</li>';
    }

    if (strlen($password_confirm) > 0 && $password != $password_confirm){
        $error_messages .= '<li>Le password non corrispondono</li>';
    }

    if(!filter_var($email, FILTER_VALIDATE_EMAIL) && strlen($email) > 0){
        $error_messages .= '<li>Email non valida</li>';
    }

    if (strpos($password, ' ') !== false && strlen($password) > 0){
        $error_messages .= '<li>La password non può contenere spazi</li>';
    }

    if (strlen($password) < 8 || strlen($password) > 20 || !preg_match('/[0-9]/', $password) || !preg_match('/[A-Z]/', $password)){
        $error_messages .= '<li>La password deve essere lunga almeno 8 caratteri e al massimo 20, deve contenere almeno un numero e una lettera maiuscola</li>';
    }
    ...
    if(!strlen($error_messages)){
        ...
        if($conn){
            if(!$funzioniDB->checkEmail($email)) {
                $error_messages .= '<li>Email già in uso</li>';
            }
            else if ($funzioniDB->registerUser($name, $surname, $email, $password, $role)){
                $ok = "Registrazione avvenuta con successo, clicca <a href=\"../index.php\">qui</a> per tornare alla home.<br/> Oppure clicca <a href=\"../php/login.php\">qui</a> per effettuare l'accesso.";
                require_once "../config.php";
                $page = file_get_contents($html_path . "registrati.html");
                $page = str_replace("<visibility/>", "<div class=\"nascosto\">", $page);
